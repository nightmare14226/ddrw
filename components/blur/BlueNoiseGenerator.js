...
export class BlueNoiseGenerator {
  ...
  generate() {
    ...
    fillWithOnes(samples.binaryPattern, pointCount);
    shuffleArray(samples.binaryPattern, this.random);
    console.timeEnd("Array Initialization");

    console.time("Score Initialization");
    for (let i = 0, l = samples.binaryPattern.length; i < l; i++) {
      if (samples.binaryPattern[i] === 1) {
