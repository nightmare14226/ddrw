...
const blueNoiseTex = new DataTexture(
  data,
  generator.size,
  generator.size,
  RGBFormat
...
export class MotionBlurPass extends Pass {
  ...
  constructor(scene, camera, options = {}) {
    ...
    this.expandGeometry =
      "expandGeometry" in options ? options.expandGeometry : 0;
    this.interpolateGeometry =
      "interpolateGeometry" in options ? options.interpolateGeometry : 1;
    this.smearIntensity =
      "smearIntensity" in options ? options.smearIntensity : 1;
    this.blurTransparent =
      "blurTransparent" in options ? options.blurTransparent : false;
    this.renderCameraBlur =
      "renderCameraBlur" in options ? options.renderCameraBlur : true;
    this.renderTargetScale =
      "renderTargetScale" in options ? options.renderTargetScale : 1;
    this.jitter = "jitter" in options ? options.jitter : 1;
    this.jitterStrategy =
      "jitterStrategy" in options
        ? options.jitterStrategy
        : MotionBlurPass.RANDOM_JITTER;
    ...
    // render targets
    this._velocityBuffer = new WebGLRenderTarget(256, 256, {
    ...
    switch (debug.display) {
      ...
      case MotionBlurPass.DEFAULT: {
        ...
        if (compositeMaterial.defines.SAMPLES !== this.samples) {
          compositeMaterial.defines.SAMPLES = Math.max(
            0,
            Math.floor(this.samples)
          );
  ...
  _getPreviousFrameState(obj) {
    ...
    const boneTextureNeedsUpdate =
      data.boneMatrices === null ||
      data.boneMatrices.length !== skeleton.boneMatrices.length;
  ...
  _drawMesh(renderer, mesh, type, saveState) {
    ...
    blurTransparent =
      "blurTransparent" in overrides
        ? overrides.blurTransparent
        : this.blurTransparent;
    renderCameraBlur =
      "renderCameraBlur" in overrides
        ? overrides.renderCameraBlur
        : this.renderCameraBlur;
    expandGeometry =
      "expandGeometry" in overrides
        ? overrides.expandGeometry
        : this.expandGeometry;
    interpolateGeometry =
      "interpolateGeometry" in overrides
        ? overrides.interpolateGeometry
        : this.interpolateGeometry;
    smearIntensity =
      "smearIntensity" in overrides
        ? overrides.smearIntensity
        : this.smearIntensity;

    const isTransparent = mesh.material.transparent || mesh.material.alpha < 1;
    const isCulled =
      mesh.frustumCulled && !this._frustum.intersectsObject(mesh);
    ...
    if (skip) {
      ...
      const material =
        type === MotionBlurPass.GEOMETRY
          ? data.geometryMaterial
          : data.velocityMaterial;
      ...
      const projMat = renderCameraBlur
        ? this._prevCamProjection
        : camera.projectionMatrix;
      const invMat = renderCameraBlur
        ? this._prevCamWorldInverse
        : camera.matrixWorldInverse;
      uniforms.prevProjectionMatrix.value.copy(projMat);
      uniforms.prevModelViewMatrix.value.multiplyMatrices(
        invMat,
        data.matrixWorld
      );
